module.exports = async page => {
  ...
  ok(await page.locator('[data-lineup-id]').count() > 0, '圣域有录像比赛可查看阵容');
  ok(await page.locator('.bracket-player:not([data-lineup-id])').count() === 0, '当前圣域比赛阵容已全部补齐');
  await page.locator('[data-lineup-id]').first().click();
  await page.locator('#lineup-panel .lineup-side').first().waitFor();
  ok(await page.locator('#lineup-panel .lineup-side').count() === 2, '圣域静态阵容加载双方数据');
  ok(await page.locator('#lineup-panel .lineup-hero').count() > 0, '圣域阵容英雄素材已渲染');
  await page.screenshot({path:'output/playwright/holyland-lineup-desktop.png',fullPage:true});
  await page.setViewportSize({width:390,height:844});
  ok(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth), '圣域阵容390px手机无页面横向溢出');
  await page.screenshot({path:'output/playwright/holyland-lineup-mobile.png',fullPage:true});
  await page.setViewportSize({width:1440,height:1050});
  await page.locator('#lineup-panel .lineup-close').click();
  ok(await page.locator('#lineup-panel').isHidden(), '圣域阵容面板可收起');
